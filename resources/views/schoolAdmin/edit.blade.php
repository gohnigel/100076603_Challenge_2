...
@section ('content')
    ...
    <form action="/schoolAdmin/{{ $schoolAdmin->id }}" method="post">
        @csrf
        @method('PUT')
        ...
        <fieldset>
            <legend>Edit school admin</legend>

            <p>
                <label for="name">Name:</label>
                <input type="text" name="name" id="name" maxlength="255" required value="{{ $schoolAdmin->name }}">
                ...
                <input type="email" name="email" id="email" required maxlength="255" value="{{ $schoolAdmin->email }}">
                <span class="manager">*</span>
            ...
            <p>
                ...
                <select name="school_id" id="school_id" required>
                    @foreach ($schools as $school)
                        <option value="{{ $school->id }}" @if( $schoolAdmin->school_id == $school->id) selected @endif>{{ $school->school_name }}</option>
                    @endforeach
                </select>
